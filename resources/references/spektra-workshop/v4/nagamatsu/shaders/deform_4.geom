...
void create(in vec3 v1, in vec3 v2, in vec3 v3){
	...
	oVert.posInProj = uTDMats[0].proj * uTDMats[0].cam * vec4(v1, 1.);
	oVert.bc = vec3(1.,0,0);
	gl_Position = oVert.posInProj;
	EmitVertex();

	oVert.posInWorld = vec4(v2, 1.);
	oVert.posInProj = uTDMats[0].proj * uTDMats[0].cam * vec4(v2, 1.);
	oVert.bc = vec3(0,1.,0);
	gl_Position = oVert.posInProj;
	EmitVertex();

	oVert.posInWorld = vec4(v3, 1.);
	oVert.posInProj = uTDMats[0].proj * uTDMats[0].cam * vec4(v3, 1.);
	oVert.bc = vec3(0,0,1.);
	gl_Position = oVert.posInProj;
	EmitVertex();
	EndPrimitive();
